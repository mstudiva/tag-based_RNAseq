# Mapping paired-end reads to transcriptomes with bowtie2, version January 24, 2022
...
# ex. "bowtie2 --local -x /mnt/beegfs/home/mstudiva/db/Host -1 R1_Host_1.fastq -2 R2_Host_1.fastq -S Host_1.fastq.sam --no-hd --no-sq --no-unal --al-conc ./Host_1.fastq.clean --un-conc junk/Host_1.fastq.un"
...
# calculate mapping efficiency from these values compared to deduplicated reads in Excel
# remember to delete the results from the .sam files
echo "countreads_host.pl '.clean' > countreads_host.txt" > count_align
